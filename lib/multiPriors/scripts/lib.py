...
#    shapes = pool.map(getSubjectShapes_parallelization, channel_mri)
    ...
    for i in range(0,len(channel_mri)):
        shapes.append(getSubjectShapes_parallelization(channel_mri[i]))
...
def sampleTestData(TPM_channel, testChannels, testLabels, subjectIndex, output_classes, output_dpatch):
...
class LossHistory_multiDice8(keras.callbacks.Callback):
    def on_train_begin(self, logs={}):
        self.losses = []
        self.dice = []
        self.metrics = []
    ...
    def on_batch_end(self, batch, logs={}):
        ...
        self.dice.append(logs.get('dice_coef_multilabel5'))
        self.dice.append(logs.get('dice_coef_multilabel6'))
        self.dice.append(logs.get('dice_coef_multilabel7'))
        self.metrics.append(self.dice)

class LossHistory_multiDice9(keras.callbacks.Callback):
    def on_train_begin(self, logs={}):
        self.losses = []
        self.dice = []
        self.metrics = []
    ...
    def on_batch_end(self, batch, logs={}):
        ...
        self.dice.append(logs.get('dice_coef_multilabel6'))
        self.dice.append(logs.get('dice_coef_multilabel7'))
        self.dice.append(logs.get('dice_coef_multilabel8'))
        self.metrics.append(self.dice)
...
def validation_on_batch(model, valbatch, TPM_patches, size_minibatches_val, vallabels, output_classes, logfile, model_name):
    ...
    for j in range(n_minibatches):
        ...
        minivalbatch_labels = np.float32(vallabels[start:end,:,:,:,:])
        if len(TPM_patches) > 0:
          ...
          if 'noDownsampling' in model_name:
            ...
            lowRes = np.zeros(output_shape,dtype='float32')
...
def train_model_on_batch(model,batch,TPM_patches,labels,size_minibatches,history,losses,metrics,logfile, output_classes, model_name):
    ...
    for j in range(n_minibatches):
        ...
        minibatch_labels = np.float32(labels[start:end,:,:,:,:])
        if len(TPM_patches) > 0:
          TPM_patch = TPM_patches[start:end,:,:,:,:]    
          if 'noDownsampling' in model_name:
...
def fullHeadSegmentation(wd, penalty_MATRIX, TPM_channel, OUTPUT_PATH, dice_compare, dsc, model, model_name, testChannels, testLabels, subjectIndex, output_classes, segmentation_dpatch, size_minibatches, softmax_output, saveSegmentation = False):    
...
##    subID = '_'.join(flairCh[0].split('/')[-3:-1])
##    subID = flairCh[0].split('/')[-2] + '_' + flairCh[0].split('/')[-1][:-1]
#    subID = flairCh[0].split('/')[-1][:-1]
#    subID=subID.split('.')[0]
#    segmentationName = subID + '_segmentation' #_epoch' + str(epoch)
#    if len(OUTPUT_PATH) > 0:
##        output = OUTPUT_PATH + '/' + segmentationName + '.nii.gz' 
#        output = OUTPUT_PATH + '/'  + segmentationName + '.nii' 
#    else:
##        output = wd + '/predictions/' + segmentationName + '.nii.gz'
#        output = wd + '/' + segmentationName + '.nii.gz'

    output = flairCh[0].split('.')[0] + '_seg.nii.gz'
...
#    my_logger('Segmenting subject with channel: ' + str(subjectGTchannel), logfile)
    ...
    TPM_patches, labels, voxelCoordinates, shape, affine = sampleTestData(TPM_channel, testChannels, testLabels, subjectIndex, output_classes, output_dpatch)
...
#      indexes.extend(class_pred)     
    ...
    if softmax_output:
      ...
      if dice_compare:           
          ...
          ch = labelsFile.readlines()
          subjectGTchannel = ch[subjectIndex[0]][:-1]
          GT = nib.load(subjectGTchannel).get_data()  
          if 0 not in GT:
            GT -= 1
#          score = weighted_generalized_dice_completeImages(GT, img.get_data(), penalty_MATRIX)
          score = generalized_dice_completeImages(img.get_data(), GT)
          dsc.append(score[0])
          print(dsc[-1])
...
#        copy(configFile[1:], wd)
        ...
        elif cfg.loss_function == 'Dice8':
            from MultiPriors_noDownsampling import Generalised_dice_coef_multilabel8, dice_coef_multilabel6, dice_coef_multilabel7, dice_coef_multilabel0,dice_coef_multilabel1,dice_coef_multilabel2,dice_coef_multilabel3,dice_coef_multilabel4,dice_coef_multilabel5
            my_custom_objects = {'Generalised_dice_coef_multilabel8':Generalised_dice_coef_multilabel8,
                                 'dice_coef_multilabel0':dice_coef_multilabel0,
                                 'dice_coef_multilabel1':dice_coef_multilabel1,
                                 'dice_coef_multilabel2':dice_coef_multilabel2,
                                 'dice_coef_multilabel3':dice_coef_multilabel3,
                                 ...
                                 'dice_coef_multilabel5':dice_coef_multilabel5,
                                 'dice_coef_multilabel6':dice_coef_multilabel6,
                                 'dice_coef_multilabel7':dice_coef_multilabel7}
        elif cfg.loss_function == 'Dice9':
            from MultiPriors_noDownsampling import Generalised_dice_coef_multilabel9, dice_coef_multilabel6, dice_coef_multilabel7, dice_coef_multilabel8, dice_coef_multilabel0,dice_coef_multilabel1,dice_coef_multilabel2,dice_coef_multilabel3,dice_coef_multilabel4,dice_coef_multilabel5
            my_custom_objects = {'Generalised_dice_coef_multilabel9':Generalised_dice_coef_multilabel9,
                                 'dice_coef_multilabel0':dice_coef_multilabel0,
                                 'dice_coef_multilabel1':dice_coef_multilabel1,
                                 'dice_coef_multilabel2':dice_coef_multilabel2,
                                 'dice_coef_multilabel3':dice_coef_multilabel3,
                                 ...
                                 'dice_coef_multilabel6':dice_coef_multilabel6,
                                 'dice_coef_multilabel7':dice_coef_multilabel7,
                                 'dice_coef_multilabel8':dice_coef_multilabel8}
        ...
        cfg.epochs_for_fullSegmentation = range(start_epoch, cfg.epochs)
        os.chdir(workingDir + '/training_sessions/')
        wd = workingDir + '/training_sessions/' +session
    ...
    elif cfg.output_classes == 8:
        history = LossHistory_multiDice8()
    elif cfg.output_classes == 9:
        history = LossHistory_multiDice9()
    for epoch in xrange(start_epoch,cfg.epochs+1):
      t1 = time.time()
      my_logger("######################################################",logfile)
...
#        list_subjects_fullSegmentation = sample(range(cfg.test_subjects), cfg.n_fullSegmentations)
        ...
        for subjectIndex in list_subjects_fullSegmentation: 
          ...
          fullHeadSegmentation(wd, cfg.penalty_MATRIX, cfg.validation_TPM_channel, cfg.OUTPUT_PATH, dice_compare, dsc, model, cfg.model, cfg.validationChannels, cfg.validationLabels, subjectIndex, \
...
#    logfile = 'segmentations.log'
    ...
    if cfg.output_classes == 6:
        ...
        try:
	        from MultiPriors_noDownsampling import Generalised_dice_coef_multilabel8, dice_coef_multilabel0,dice_coef_multilabel1,dice_coef_multilabel2,dice_coef_multilabel3,dice_coef_multilabel4,dice_coef_multilabel5, dice_coef_multilabel6, dice_coef_multilabel7
	        my_custom_objects = {'Generalised_dice_coef_multilabel8':Generalised_dice_coef_multilabel8,
				     'dice_coef_multilabel0':dice_coef_multilabel0,
				     'dice_coef_multilabel1':dice_coef_multilabel1,
				     'dice_coef_multilabel2':dice_coef_multilabel2,
        ...
        try:
	        from MultiPriors_noDownsampling import Generalised_dice_coef_multilabel9, dice_coef_multilabel0,dice_coef_multilabel1,dice_coef_multilabel2,dice_coef_multilabel3,dice_coef_multilabel4,dice_coef_multilabel5, dice_coef_multilabel6, dice_coef_multilabel7, dice_coef_multilabel8
	        my_custom_objects = {'Generalised_dice_coef_multilabel9':Generalised_dice_coef_multilabel9,
				     'dice_coef_multilabel0':dice_coef_multilabel0,
				     'dice_coef_multilabel1':dice_coef_multilabel1,
				     'dice_coef_multilabel2':dice_coef_multilabel2,
...
#    epoch = start_epoch
    ...
    if list_subjects_fullSegmentation == []:
      list_subjects_fullSegmentation = range(cfg.test_subjects)
    for subjectIndex in list_subjects_fullSegmentation:        
#        fullHeadSegmentation(wd, cfg.penalty_MATRIX, cfg.TPM_channel, cfg.OUTPUT_PATH, dice_compare, dsc, model, cfg.model, cfg.segmentChannels, cfg.segmentLabels, subjectIndex, \
#        cfg.output_classes,cfg.segmentation_dpatch, cfg.size_test_minibatches, logfile, epoch, cfg.softmax_output, cfg.saveSegmentation)
        fullHeadSegmentation(wd, cfg.penalty_MATRIX, cfg.TPM_channel, cfg.OUTPUT_PATH, dice_compare, dsc, model, cfg.model, cfg.segmentChannels, cfg.segmentLabels, subjectIndex, \
        cfg.output_classes,cfg.segmentation_dpatch, cfg.size_test_minibatches, cfg.softmax_output, cfg.saveSegmentation)
#        my_logger('--------------- TEST EVALUATION ---------------', logfile)
#        my_logger('          Full segmentation evaluation of subject' + str(subjectIndex), logfile)
