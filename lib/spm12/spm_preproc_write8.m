...
% Modified for ROAST by Yu (Andy) Huang  2018-09-05
...
% rHoles = spm_load_mask(fullfile(fileparts(which(mfilename)),'OpticNerveHoles.nii'));
% rEyes = spm_load_mask(fullfile(fileparts(which(mfilename)),'Eyes.nii'));
% % andy 2017-04-17
rHoles = spm_load_mask(fullfile(fileparts(which(mfilename)),'eTPM_Skull Holes.nii'));
...
holes_vol = zeros(d);
eyes_vol = zeros(d);
WMexclude_vol = zeros(d);
% manually-specified volume for doing auto-patching locally % andy 2017-04-17
    ...
    if do_defs
        ...
        if do_cls
            ...
            if isfield(res,'mg')
                ...
                holes   = spm_sample_mask(rHoles,t1,t2,t3);
                eyes   = spm_sample_mask(rEyes,t1,t2,t3); % andy 2017-04-17
                WMexclude = spm_sample_mask(rWMexclude,t1,t2,t3);
                wp  = res.wp;
                s   = zeros(size(b{1}));
                for k1 = 1:Kb
                    b{k1} = wp(k1)*b{k1};
                ...
                holes_vol(:,:,z) = holes{1};
                eyes_vol(:,:,z) = eyes{1}; % andy 2017-04-17
                WMexclude_vol(:,:,z) = WMexclude{1};
            else
...
% save(fullfile([pth filesep nam '_rmask.mat']),'holes_vol','eyes_vol','WMexclude_vol'); % andy 2017-04-17
